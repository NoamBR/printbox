...
$tasks = @(
    ...
    # HERO ADVERTISING COMPOSITE — landing page hero focal image
    @{ id="HERO"; type="cups_splash"; prompt="Hero advertising photography of two matte black 16oz cold drink paper cups standing slightly off-center on a polished concrete pedestal, each cup wrapped with a polished gold foil minimalist monogram and a thin gold geometric ring near the rim, clear PET dome lids each pierced by a slim matte black paper straw, a dynamic mid-air liquid splash arcs between the cups with crisp frozen droplets and ribbons of liquid suspended in time, the splash catches warm gold rim light from the upper left while the cups remain in dramatic chiaroscuro shadow, dark concrete-grey gradient background fading from #0A0A0A in the center to #1C1C1C at the edges with subtle vignette, sharp tack-focus on the cups and the central splash, very slight motion blur only on the outermost droplets, ultra-detailed 8k commercial product photography, editorial luxury beverage advertising aesthetic, cinematic anamorphic feel, no people, no text, no logo overlays beyond the gold monogram on the cups" }
)
